//////////////////////////////////////////////
// update.js (merged textures + logic)
//////////////////////////////////////////////
...
function pGradientH(){
  ...
  for(let i=0; i<steps; i++){
    ...
    pGradH.stroke(c); pGradH.line(i*2, 0, i*2, pGradH.height);
  }
}
...
function pGradientV(){
  ...
  for(let i=0; i<steps; i++){
    ...
    pGradV.stroke(c); pGradV.line(0, i*2, pGradV.width, i*2);
  }
}
...
function pGradientCH(){
  ...
  for(let i=0; i<steps; i++){
    ...
    pGradCH.stroke(c); pGradCH.line(i*2, 0, i*2, pGradCH.height);
  }
}

// ----------------- LAYOUT -----------------
function setText(){
  pgT=[]; pEntry=[]; heightRatio=[]; xNudge=[]; wordCount=[]; sHarry=[]; fullHeight=0;
  ...
  for(let k=0; k<keyArray.length; k++){
    if(lineDist > wrapWidth){
      xNudge[lineCount]=lineDist; wordCount[lineCount]=thisWordCount; sHarry[lineCount]=sH; fullHeight+=sH;
      lineCount++; lineDist=0; thisWordCount=0;
      rSh=random(10); sH=stripH;
      if(rSh>7.5) sH=stripH/4; else if(rSh>5) sH=stripH/2;
    }

    let ver=0; const token=keyArray[k];
    ...
    else { const rFont=random(10); if(rFont<8) pgTexture1(token,0,k,sH); else pgTexture1(token,typeToggle,k,sH); ver=9; }
...
function aSet(t,influ){ const c=t%1; return pow(c,influ)/(pow(c,influ)+pow(1-c,influ)); }

function randomInsert(){
  const add=(sym,n)=>{for(let r=0;r<n;r++){const i=round(random(keyArray.length));keyArray.splice(i,0,sym);}};
  add("X0",1+floor(keyArray.length/5));
  add("X1",1+floor(keyArray.length/12));
  add("X2",1+floor(keyArray.length/12));
  add("X3",1+floor(keyArray.length/12));
  add("X4",1+floor(keyArray.length/18));
  add("X5",1+floor(keyArray.length/10));
  add("X6",1+floor(keyArray.length/15));
  add("X7",1+floor(keyArray.length/12));
  add("X8",floor(keyArray.length/15));
}

function hideWidget(){ widgetOn=!widgetOn; document.getElementById('widget').style.display=widgetOn?"block":"none"; }
function invert(){ inverter=!inverter;
  if(inverter){ bkgdColor=color('#fff'); foreColor=color('#000'); } else { bkgdColor=color('#000'); foreColor=color('#fff'); }
  colorA[4]=bkgdColor; pGradientH(); pGradientV(); pGradientCH(); setText();
}
function setWpadding(v){ wPad=v; wWindow=width-map(wPad,0,100,0,width); setText(); }
function setStripH(v){ stripH=round(v); setText(); }

// ----------------- EXPORT -----------------
window.setText=setText; window.reRoll=reRoll; window.aSet=aSet; 
window.randomInsert=randomInsert; window.hideWidget=hideWidget;
window.invert=invert; window.setWpadding=setWpadding; window.setStripH=setStripH;
window.pgTexture1=pgTexture1; window.pgImage=pgImage;
window.pSlash=pSlash; window.pRound=pRound; window.pBlank=pBlank;
window.pGradientH=pGradientH; window.pGradientV=pGradientV; window.pGradientCH=pGradientCH;
